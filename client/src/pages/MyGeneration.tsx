...
const MyGeneration = () => {
  ...
  const fetchThumbnails = async () => {
    try {
      setLoading(true);
      const { data } = await api.get('/user/thumbnails'); // ✅ removed /api prefix
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!confirm) return;
      const { data } = await api.delete(`/thumbnail/delete/${id}`); // ✅ fixed route
      toast.success(data.message);
      setThumbnails(prev => prev.filter((t) => t._id !== id)); // ✅ removed extra fetchThumbnails call
    } catch (error: any) {
      console.log(error);
      toast.error(error?.response?.data?.message || error.message);
    }
            ...
            {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                <div
                  ...
                  className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid" // ✅ fixed breaking-inside-avoid typo
                >
                  {/* Image */}
